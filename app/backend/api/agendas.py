...
from app.backend.schemas.agenda_excepcional import (
    AgendaExcepcionalCreate,
...
# Función de Inyección de Dependencia (Define la cadena de dependencias)
...
# ----------------------------------------------------
# Endpoint para Agenda Excepcional (POST)
# ----------------------------------------------------
@router.post(
    "/medicos/{matricula}/agenda/excepcional",
    response_model=AgendaExcepcionalOut,
...
# ----------------------------------------------------
# Endpoint para Agenda Regular (POST)
# ----------------------------------------------------
@router.post(
    "/medicos/{matricula}/agenda/regular",
    response_model=AgendaRegularOut,
    status_code=status.HTTP_201_CREATED,
)
...
# ----------------------------------------------------
# Endpoint para Consulta de Agendas Regulares (GET)
# ----------------------------------------------------
@router.get(
    "/medicos/{matricula}/agenda/regular", response_model=List[AgendaRegularOut]
)
def obtener_agendas_regulares(
    matricula: str, service: AgendaService = Depends(get_agenda_service)
):
    try:
...
# ----------------------------------------------------
# Endpoint para Eliminar Agenda Regular (DELETE)
# ----------------------------------------------------
@router.delete(
    "/medicos/{matricula}/agenda/regular/{agenda_id}",
...
def eliminar_agenda_regular(
    ...
    try:
        service.eliminar_agenda_regular(
            matricula, especialidad_id, dia_de_semana, hora_inicio
        )
