...
class AgendaRepository:
    ...
    def create_agenda_regular(self, medico_matricula: str, data: AgendaRegularCreate) -> AgendaRegular:
        # ... (Tu código de creación se mantiene) ...
        agenda = AgendaRegular(Medico_Matricula=medico_matricula, Especialidad_Id=data.Especialidad_Id, Dia_de_semana=data.Dia_de_semana, Hora_inicio=data.Hora_inicio, Hora_fin=data.Hora_fin, Duracion=data.Duracion, Sucursal_Id=data.Sucursal_Id)
        self.db.add(agenda)
        self.db.commit()
        self.db.refresh(agenda)
        return agenda
    
    def get_agendas_regulares_by_medico(self, medico_matricula: str) -> List[AgendaRegular]:
        return self.db.query(AgendaRegular).filter(AgendaRegular.Medico_Matricula == medico_matricula).all()

    def get_agenda_regular_by_pk(self, medico_matricula: str, dia_de_semana: int, hora_inicio: str) -> AgendaRegular | None:
        return self.db.query(AgendaRegular).filter(AgendaRegular.Medico_Matricula == medico_matricula, AgendaRegular.Dia_de_semana == dia_de_semana, AgendaRegular.Hora_inicio == hora_inicio).first()
    ...
    def verificar_disponibilidad(self, medico_matricula: str, fecha_turno: date, hora_turno: time, duracion: int) -> str | None:
        ...
        bloqueo = self.db.query(AgendaExcepcional).filter(
            ...
            AgendaExcepcional.Es_Disponible == 0, # Marcado como NO disponible (Bloqueo)
        ...
        disponibilidad_excepcional = self.db.query(AgendaExcepcional).filter(
            AgendaExcepcional.Medico_Matricula == medico_matricula,
        ...
        cobertura_regular = self.db.query(AgendaRegular).filter(
        ...
        agenda_solo_por_dia = self.db.query(AgendaRegular).filter(
            AgendaRegular.Medico_Matricula == medico_matricula,
            AgendaRegular.Dia_de_semana == dia_semana
        ).first()
