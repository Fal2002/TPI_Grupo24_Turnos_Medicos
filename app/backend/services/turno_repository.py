...
class TurnoRepository:
    ...
    def get_by_pk(self, fecha: str, hora: str, paciente_nro: int) -> Optional[Turno]: # Corregido: Turno | None
    ...
    def get_turnos_by_medico_and_dates(self, medico_matricula: str, start_date: str, end_date: str) -> List[dict]:
        """
        ...
        query = query.filter(
            ...
            and_(
                Turno.Fecha >= start_date,
                Turno.Fecha <= end_date
            )
    ...
    def count_turnos_by_especialidad(self, start_date: str, end_date: str) -> List[dict]:
        """
        ...
        query = query.join(Especialidad, Especialidad.Id_especialidad == Turno.Especialidad_Id) # CLAVE CORREGIDA
        query = query.filter(
            and_(
                Turno.Fecha >= start_date,
                Turno.Fecha <= end_date
    ...
    def get_pacientes_atendidos(self, start_date: str, end_date: str) -> List[dict]:
        ...
        estados_finales = self.db.query(Estado).filter(
            Estado.Descripcion.in_(['Finalizado', 'Atendido'])
        ).all()
    ...
    def count_asistencia_vs_inasistencia(self, start_date: str, end_date: str) -> List[dict]:
        """
        Cuenta el total de turnos en estado 'Finalizado' (Asistencia) y 'Ausente' (Inasistencia).
        """
        
        estados_interes = self.db.query(Estado).filter(
            Estado.Descripcion.in_(['Finalizado', 'Ausente'])
        ).all()
    ...
    def get_turnos_by_time_range(self, target_date: date, start_time: time, end_time: time) -> List[Turno]:
