...
from app.backend.core.security import hash_password # Para hashear la contraseña temporal
from app.backend.services.exceptions import (
    MatriculaDuplicadaError, RecursoNoEncontradoError, EmailDuplicadoError
) 
...
class MedicoService:
    ...
    def crear_medico(self, data: MedicoCreate) -> Medico:
        ...
        if self.user_repo.get_by_email(data.email_login):
            raise EmailDuplicadoError(f"El email {data.email_login} ya está registrado como usuario.")

        # --- CREACIÓN DEL USER ---
        role_medico = self.user_repo.get_role_by_name("Médico")
        if not role_medico:
            raise RecursoNoEncontradoError("El rol 'Médico' no está configurado en la base de datos.")
