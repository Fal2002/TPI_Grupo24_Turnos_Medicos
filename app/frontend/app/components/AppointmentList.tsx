...
// ⬇️ ORDENAR ANTES DE SETEAR STATE
...
setAppointments(enriched);
  ...
  const { upcomingAppointments, pastAppointments } = useMemo(() => {
    ...
    appointments.forEach(app => new Date(`${app.Fecha}T${app.Hora}`) >= now ? upcoming.push(app) : past.push(app));
  ...
  const handleConfirmCancel = async () => {
    if (selectedAppointment) {
      try {
        await cambiarEstadoTurno({fecha: selectedAppointment.Fecha, hora: selectedAppointment.Hora, paciente_nro: paciente!.nroPaciente}, 'cancelar');
      } catch (error) {
        ...
        <div className={`p-5 rounded-lg shadow-md border-l-4 flex flex-col md:flex-row md:items-center md:justify-between transition-all ${styles.container} ${appointment.estado === 'Cancelado' || appointment.estado === 'Finalizado' ? 'bg-gray-50' : 'bg-white'}`}>
          <div className="flex-grow">
            ...
            <div className="mt-2 space-y-1 text-sm text-gray-600">
              <p className="flex items-center gap-2"><Stethoscope size={16} /> {`${appointment.Medico_Nombre} ${appointment.Medico_Apellido}`}</p>
              <p className="flex items-center gap-2 font-semibold"><Calendar size={16} /> {formatDateTime(`${appointment.Fecha}T${appointment.Hora}`)}</p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md text-center">
            ...
            <p className="text-gray-600 mb-4">
              ...
              <span className="font-semibold">{selectedAppointment.Especialidad_Id}</span> con{' '}
              <span className="font-semibold">{selectedAppointment.Medico_Matricula}</span>?
            </p>
