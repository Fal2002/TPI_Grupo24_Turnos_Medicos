...
export async function deleteConsultorio(numero: number, sucursal_id: number) {
  const response = await fetch(`${API_URL}/${numero}/${sucursal_id}`, {
    method: "DELETE",
  });
  if (!response.ok) {
...
export async function updateConsultorio(numero: number, sucursal_id: number, data: any) {
  const response = await fetch(`${API_URL}/${numero}/${sucursal_id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
